...
public class HangmanCanvas_extension extends GCanvas{
	...
	public void drawman(int i) {
		if(Hangman_extension.done==false)
		{
			...
			if (i < 7) {
				...
				if (i < 6) {
					...
					GLine LUA = new GLine(getWidth() / 2,
							4 * getHeight() / 5 - SCAFFOLD_HEIGHT + ROPE_LENGTH + HEAD_RADIUS + ARM_OFFSET_FROM_HEAD,
					...
					if (i < 5) {
						...
						if (i < 4) {
							...
							GLine LL = new GLine(getWidth() / 2 - HIP_WIDTH / 2,
									4 * getHeight() / 5 - SCAFFOLD_HEIGHT + ROPE_LENGTH + BODY_LENGTH + HEAD_RADIUS,
									getWidth() / 2 - HIP_WIDTH / 2, 4 * getHeight() / 5 - SCAFFOLD_HEIGHT + ROPE_LENGTH
											+ BODY_LENGTH + HEAD_RADIUS + LEG_LENGTH);
							add(LL);
							if (i < 3) {
		...
		else if (Hangman_extension.done==true)
		{
			remove(wrongs);
			remove(curr);
			remove(correct);
	...
	public void noteIncorrectGuess(char letter) {
		...
		;//wesit aq shemedzlo letteris shemotana, magram isedac kargi string mqonda amistvis, tore sheidzlebs string=string+letter
